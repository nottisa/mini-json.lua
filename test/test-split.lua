...
test("numbers", function()
  ...
  assert( decode("13e2") == 13e2 )
  assert( decode("13E+2") == 13e2 )
  assert( decode("13e-2") == 13e-2 )
end)
...
test("arrays", function()
  local t = { "cat", "dog", "owl" }
  assert( equal( t, decode( encode(t) ) ) )
end)
...
test("decode invalid string", function()
  ...
  for i, v in ipairs(t) do
    local status, err = pcall(decode, v)
    assert( not status, fmt("'%s' was parsed without error", v) )
  end
end)
...
test("encode invalid", function()
  ...
  for i, v in ipairs(t) do
    local status, res = pcall(encode, v)
